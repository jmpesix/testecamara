...
export default function PortalVereador() {
  ...
  const [mainView, setMainView] = useState<'all' | 'vereadores' | 'duvidas' | 'reclamacoes' | 'resolved'>('all');
  ...
  const filteredMessages = messages.filter(msg => {
    ...
    if (mainView === 'duvidas' && !sourceLower.includes('duvida') && !sourceLower.includes('informação')) return false;
    if (mainView === 'reclamacoes' && !sourceLower.includes('reclamação')) return false;
  ...
  return (
    <div className="flex h-screen bg-[#f4f1ea] text-[#0a192f] font-sans selection:bg-[#c5a059]/30 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-8">
          <div>
            ...
            <div className="space-y-1">
              {[
                ...
                { id: 'duvidas', icon: AtSign, label: 'Atendimento Público', sublabel: 'Informações Oficiais', color: 'text-emerald-400' },
                { id: 'reclamacoes', icon: Clock, label: 'Ouvidoria Geral', sublabel: 'Reclamações e Críticas', color: 'text-amber-400' },
                { id: 'resolved', icon: CheckCheck, label: 'Arquivo Permanente', sublabel: 'Casos Resolvidos', color: 'text-slate-400' }
              ].map((item) => {
                const itemCount = messages.filter(msg => {
                  ...
                  if (item.id === 'duvidas') return sourceLower.includes('duvida') || sourceLower.includes('informação');
                  if (item.id === 'reclamacoes') return sourceLower.includes('reclamação');
                  return true;
                }).length;
          ...
          <div>
            ...
            <div className="space-y-1 px-1">
              {teams.map((team, idx) => (
                <button key={idx} className="w-full flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-white/5 text-slate-500 hover:text-slate-200 transition-all text-xs font-bold group">
                  <div className="w-2 h-2 rounded-full bg-[#c5a059]/30 group-hover:bg-[#c5a059]" />
                  {team.name}
                </button>
              ))}
            </div>
      ...
      <div className="w-[400px] bg-white border-r border-[#c5a059]/20 flex flex-col z-10 shadow-xl">
        <div className="p-8 border-b border-[#c5a059]/10 bg-[#fdfcf9]">
          <div className="flex items-center justify-between mb-6">
            ...
            <button className="p-3 bg-[#0a192f] hover:bg-[#152a4a] rounded-xl text-white transition-all shadow-xl shadow-blue-900/20 group">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-1.5 text-[9px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">
                        <div className={`w-1.5 h-1.5 rounded-full ${msg.status === 'open' ? 'bg-[#c5a059]' : 'bg-slate-300'}`} />
                        Câmara de SJB
                      </div>
                    </div>
                  </div>
      ...
      <div className="flex-1 flex flex-col bg-[#f4f1ea] relative">
        {selectedMessage ? (
                ...
                {aiSuggestion && (
                  ...
                  >
                    <div className="flex items-center gap-4 mb-6 text-emerald-700">
                      ...
                      <div>
                        <h4 className="font-serif font-bold text-xl">Recomendação da Chancelaria IA</h4>
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-20 text-center">
            ...
            <h3 className="font-serif text-4xl font-bold text-[#0a192f] mb-6 tracking-tight italic">Chancelaria de Demandas</h3>
      ...
      {/* Right Sidebar - Institutional Details */}
      <AnimatePresence>
        {selectedMessage && showContactDetails && (
          <motion.div 
            ...
            className="bg-white border-l border-[#c5a059]/20 flex flex-col overflow-hidden z-10 shadow-2xl"
          >
            <div className="flex-1 overflow-y-auto custom-scrollbar">
              <div className="p-10 border-b border-[#c5a059]/10 text-center bg-[#fdfcf9]">
                <div className="w-24 h-24 bg-white rounded-[32px] mx-auto flex items-center justify-center text-3xl font-serif font-bold text-[#c5a059] border-2 border-[#c5a059]/20 mb-6 shadow-xl">
                  {selectedMessage.contact_name ? selectedMessage.contact_name[0] : 'P'}
                </div>
                <h3 className="text-[#0a192f] font-serif font-bold text-2xl tracking-tight">{selectedMessage.contact_name || 'Protocolo Reservado'}</h3>
                <div className="inline-block mt-3 px-4 py-1 bg-[#c5a059]/10 rounded-full border border-[#c5a059]/20">
                  <span className="text-[10px] font-black text-[#c5a059] uppercase tracking-widest">ID do Peticionário: #{selectedMessage.contact_id || '---'}</span>
                </div>
              </div>
              ...
              <div className="p-10 space-y-12">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {(['urgent', 'high', 'medium', 'low'] as const).map((p) => (
                      <button
                        ...
                        className={`py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all border ${
                          selectedMessage.priority === p 
                            ? 'bg-[#0a192f] text-[#c5a059] border-[#c5a059]/50 shadow-xl' 
                            : 'bg-white text-slate-400 border-slate-100 hover:border-[#c5a059]/30'
                        }`}
                      >
                        {p}
                      </button>
                    ...
                    <button
                      ...
                      className={`col-span-2 py-2 rounded-xl text-[8px] font-black uppercase tracking-[0.2em] transition-all border ${
                        !selectedMessage.priority 
                          ? 'bg-slate-700 text-white border-slate-600' 
                          : 'bg-white text-slate-300 border-dashed border-slate-200 hover:text-slate-500'
                      }`}
                    >
                ...
                <div className="space-y-4">
                  <div className="flex flex-wrap gap-2 mb-6">
                    {selectedMessage.labels && selectedMessage.labels.length > 0 ? (
                      selectedMessage.labels.map((label, idx) => (
                        <span key={idx} className="bg-[#c5a059]/10 text-[#c5a059] text-[9px] px-3 py-1.5 rounded-full font-black border border-[#c5a059]/20 uppercase tracking-widest">
                          {label}
                        </span>
                      ))
                    ) : (
                      <span className="text-[10px] text-slate-400 font-serif italic">Nenhuma indexação oficial aplicada.</span>
                    )}
                  </div>
                  <div className="flex gap-2">
                    <input 
                      ...
                      className="flex-1 bg-[#f4f1ea] border border-[#c5a059]/20 rounded-2xl px-5 py-3 text-xs outline-none focus:border-[#c5a059]/50 transition-all font-bold placeholder:text-slate-400"
                    />
                    <button 
                      onClick={handleAddLabel}
                      className="p-3 bg-[#0a192f] text-[#c5a059] rounded-2xl border border-[#c5a059]/30 hover:bg-[#152a4a] transition-all"
                    >
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-2">
                    {(['open', 'resolved', 'pending', 'snoozed'] as const).map((s) => (
                      ...
                      >
                        {s === 'resolved' ? 'Arquivado' : s === 'open' ? 'Ativo' : s}
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-6 pt-6 border-t border-[#c5a059]/10">
                  <div className="space-y-4">
                    <div className="flex justify-between items-center text-[11px]">
                      <span className="text-slate-500 font-bold">Relator</span>
                      <span className="text-[#0a192f] font-black uppercase tracking-tighter bg-[#c5a059]/10 px-3 py-1 rounded-lg border border-[#c5a059]/10">{selectedMessage.assignee || 'Não atribuído'}</span>
                    </div>
                    <div className="flex justify-between items-center text-[11px]">
                      <span className="text-slate-500 font-bold">Fonte Oficial</span>
                      <span className="text-[#0a192f] font-black uppercase tracking-widest truncate ml-6">Câmara de SJB</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-6 pt-6 border-t border-[#c5a059]/10">
                  <div className="space-y-4">
                    {selectedMessage.custom_attributes && Object.entries(selectedMessage.custom_attributes).map(([key, value]) => (
                      <div key={key} className="flex flex-col gap-2 p-5 bg-[#fcfaf5] border border-[#c5a059]/10 rounded-[28px] group hover:border-[#c5a059]/30 transition-all">
                        <span className="text-[9px] text-[#c5a059] font-black uppercase tracking-widest opacity-70">{key.replace(/_/g, ' ')}</span>
                        <span className="text-xs font-bold text-[#0a192f] leading-relaxed">{String(value)}</span>
                      </div>
                    ))}
                    <button 
                      onClick={() => {
                        const key = prompt('Título do Atributo:');
                        const value = prompt('Valor:');
                        if (key && value) {
                          fetch(`/api/chatwoot/conversations/${selectedMessage.conversation_id}/custom_attributes`, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                      ...
                      className="w-full py-4 border-2 border-dashed border-[#c5a059]/20 rounded-[28px] text-[10px] text-[#c5a059] hover:bg-[#c5a059]/5 transition-all uppercase font-black tracking-widest"
                    >
                      + Novo Atributo Oficial
                    </button>
                  </div>
                </div>
              </div>
            </div>
            <div className="p-10 border-t border-[#c5a059]/10 bg-[#fdfcf9] text-center">
              <p className="text-[10px] text-slate-400 italic font-medium leading-relaxed">
                Informações restritas ao uso da Câmara Municipal de São João da Barra. Acesso auditado.
              </p>
            </div>
          </motion.div>
        )}
